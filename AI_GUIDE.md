...
- Todo window added: maximized todo list with hierarchical tasks. Tasks displayed with multiple colored bars showing full path from root to current level - each task shows all bars from level 0 (purple) to its own level (rainbow: purple, blue, cyan, green, yellow, orange, red). Current task bar is 6px wide, parent bars are 2px wide but each occupies 6px space (centered with 2px margins on each side). Text is white (dark theme). Rich text editing with auto-resize, content saved automatically on input with debounce. Focus preserved during updates. Height properly restored for multi-line tasks on reload. Closing todo window also closes timer overlay. Action buttons appear on task click: "Добавить подзадачу" (creates child task), "Выполнить" (starts timer), "Выполнено" (marks as completed and hides). Demo tasks created automatically on first run. Can load tasks from JSON file via IPC.
